...
public class ModelVulture extends ModelBase
{
	...
	public ModelVulture(float par1)
	{
		...
		upperBeak.addBox( -2.5F, -1F, -5F, 5, 2, 8);
		upperBeak.setRotationPoint(0, -0.8F, -10F);
		upperBeakTip = new ModelRenderer( this, 72, 118 );
		upperBeakTip.setTextureSize( 128, 128 );
		upperBeakTip.addBox( -1F, -1F, -1F, 2, 2, 2);
		upperBeakTip.setRotationPoint(0, 0, -6);
		lowerBeak = new ModelRenderer( this, 80, 118 );
		lowerBeak.setTextureSize( 128, 128 );
		lowerBeak.addBox( -2.5F, -1F, -5F, 5, 2, 8);
		lowerBeak.setRotationPoint(0, 1.5F, -10F);
		lowerBeakTip = new ModelRenderer( this, 78, 121 );
		lowerBeakTip.setTextureSize( 128, 128 );
		lowerBeakTip.addBox( -1F, -0.5F, -1F, 2, 1, 2);
		lowerBeakTip.setRotationPoint(0, -0.5F, -6F);
		...
		beakMap.put(BonesMouth.UPPER_MOUTH, upperBeak);
		beakMap.put(BonesMouth.LOWER_MOUTH, lowerBeak);
		animationBeak = new ModelAnimator<BonesMouth>(beakMap, AnimationRegistry.instance().getAnimation("bird_beak"));
	...
	public void setFlyingAnimations(AnimationState wingState, AnimationState legState, AnimationState beakState, float roll, float headYaw, float headPitch, float parTick)
	{
		...
		animationBeak.updateAnimation(beakProgress);
		if(legAction == AnimationAction.RUN)
		{
			// 60=min, 93=max, 126=min, 159=max
			if(legProgress >= 38F / (331F + 17F) && legProgress < (170F + 17F) / (331F + 17F))
			{
				legProgress += (51 - 38) / (331F + 17F); // Sync frame 38 to frame 60
				if(legProgress >= (170F + 17F) / (331F + 17F))
					legProgress -= (float)((170 + 17) - 38) / 348F;
				
				float t = (float)Math.PI * 2 * 4 * legProgress / (1.0F - (38F / 348F)); // period = half of run cycle
				body.rotateAngleX += (float)(-Math.cos(t) * 0.04);
				neck1.rotateAngleX += (float)(Math.cos(t) * 0.08);
				body.rotationPointY += -(float)(Math.cos(t) * 1.9);
			}
		}
	...
	public void setRotationAngles(float limbPeriod, float limbMaxMovement, float ticksExisted, float headYaw, float entityPitch, float unitScale, Entity entity)
	{
        ...
        float pitchFactor = entityPitch / 50F;
        if(pitchFactor > 1.0F)
        	pitchFactor = 1.0F;
        else if(pitchFactor < 0) // We only want positive pitch
        	pitchFactor = 0;
        
        body.rotationPointY += 20 - 20 * pitchFactor;
	}
	...
	public void resetSkeleton()
	{
		...
		setRotation(tail, 0.3F, 0, 0);
		setRotation(upperBeak, 0, 0, 0);
		setRotation(lowerBeak, 0, 0, 0);
		
		body.setRotationPoint(0,  -19F, 0);
	}
